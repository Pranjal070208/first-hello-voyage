...
const BenefitsPage: React.FC = () => {
  ...
  const studentsSection = useScrollAnimation();
  const winnersSection = useScrollAnimation();
  const membersSection = useScrollAnimation();
  const schoolsSection = useScrollAnimation();
  const finalSection = useScrollAnimation();
      ...
      <section className="py-12 sm:py-20 bg-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div 
            ...
            className={`text-center mb-12 sm:mb-16 scroll-reveal ${heroSection.isVisible ? 'visible' : ''}`}
          >
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
              {[
                ...
                <div key={index} className={`bg-gradient-to-br from-blue-100 to-blue-300 p-6 rounded-xl shadow-xl hover:scale-105 transform transition-all scroll-reveal ${studentsSection.isVisible ? 'visible' : ''} stagger-${index + 1}`}>
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
              {[
                ...
                <div key={index} className={`bg-gradient-to-br from-green-100 to-green-300 p-6 rounded-xl shadow-xl hover:scale-105 transform transition-all scroll-reveal ${winnersSection.isVisible ? 'visible' : ''} stagger-${(index % 6) + 1}`}>
                  <div className="text-4xl mb-4">{item.icon}</div>
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
              {[
                ...
                <div key={index} className={`bg-gradient-to-br from-purple-100 to-purple-300 p-6 rounded-xl shadow-xl hover:scale-105 transform transition-all scroll-reveal ${membersSection.isVisible ? 'visible' : ''} stagger-${index + 1}`}>
                  <div className="text-4xl mb-4">{item.icon}</div>
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
              {[
                ...
                <div key={index} className={`bg-gradient-to-br from-yellow-100 to-yellow-300 p-6 rounded-xl shadow-xl hover:scale-105 transform transition-all scroll-reveal ${schoolsSection.isVisible ? 'visible' : ''} stagger-${index + 1}`}>
                  <div className="text-4xl mb-4">{item.icon}</div>
          ...
          <div 
            ref={finalSection.elementRef}
            className={`text-center scroll-reveal ${finalSection.isVisible ? 'visible' : ''}`}
          >
            <img
              src="https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=1200&q=80"
              alt="Startup competition winners celebration"
              className="rounded-2xl shadow-2xl mx-auto w-full max-w-4xl hover:scale-105 transition-all mb-8 animate-zoom-in"
            />
          </div>
        </div>
      </section>
