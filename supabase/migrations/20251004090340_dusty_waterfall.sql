/*
  # Create marketplace tables and user tokens system

  1. New Tables
    - `user_tokens` - Stores IFG token balances for each user
    - `marketplace_products` - Products listed in the marketplace
    - `marketplace_transactions` - Purchase history and token transfers

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users
    - Automatic token assignment for new users

  3. Features
    - 10,000 IFG tokens for new users
...
-- Function to automatically create user tokens for new users
...
-- Create tokens for existing users (run once)
INSERT INTO user_tokens (user_id, balance)
SELECT id, 10000 FROM auth.users
ON CONFLICT (user_id) DO NOTHING;